        ...
        if ($_POST['remove-request'] == "remove") {
          switch (removeCategory($_POST['remove-category'])) {
            case "remove-category-success":
              $alerts = array(
                "Categoría eliminada correctamente. " => "alert-success"
              );
              break;
            case "remove-category-failure":
              $alerts = array(
                "Ha ocurrido un fallo al eliminar la categoría, compruebe que no tenga convocatorias o inténtelo de nuevo o contacte a soporte@toposiciones.com" => "alert-danger"
              );
              break;
            default:
              break;
        ...
        if ($_POST['add-request'] == "add") {
          switch (addCategory($_POST)) {
            case "add-category-success":
              $alerts = array(
                "Categoría añadida correctamente. " => "alert-success"
              );
              break;
            case "add-category-failure":
              $alerts = array(
                "Ha ocurrido un fallo al añadir la categoría, inténtelo de nuevo o contacte a soporte@toposiciones.com" => "alert-danger"
              );
              break;
            default:
              break;
