        ...
        if ($_POST['remove-request'] == "remove") {
          switch (removeAmbit($_POST['remove-ambit'])) {
            case "remove-ambit-success":
              $alerts = array(
                "Ámbito eliminado correctamente. " => "alert-success"
              );
              break;
            case "remove-ambit-failure":
              $alerts = array(
                "Ha ocurrido un fallo al eliminar el ámbito, compruebe que no tenga categorías o inténtelo de nuevo o contacte a soporte@toposiciones.com" => "alert-danger"
              );
              break;
            default:
              break;
      ...
      if ($_POST['add-request'] == "add") {
        switch (addAmbit($_POST)) {
          case "add-ambit-success":
            $alerts = array(
              "Ámbito creado correctamente. " => "alert-success"
            );
            break;
          case "add-ambit-failure":
            $alerts = array(
              "Ha ocurrido un fallo al crear el ámbito, inténtelo de nuevo o contacte a soporte@toposiciones.com" => "alert-danger"
            );
            break;
          case "add-ambit-failure-alreadyexists":
            $alerts = array(
              "Ha ocurrido un fallo al crear el ámbito, ya existe" => "alert-warning"
            );
            break;
          default:
            break;
